...
class LandmarkHead(nn.Module):
    ...
    def forward(self,x):
        ...
        # [b,h,w,2*10]------>[b,h*w*2,10]  5个关键点,5个x5个y
...
class RetinaFace(nn.Module):
    def __init__(self, cfg = None, pretrained = False, mode = 'train'):
        ...
        # a class forward get order-dict  1:[b,h//8,w//8,32]...2:[]
        # get backbone later no fc and no gap layer
    ...
    def forward(self,inputs):
        ...
        # [batch_size,80*80*2+40*40*2+20*20*2,4] ------>[b,16800,4]
        # [batch_size,80*80*2+40*40*2+20*20*2,2] ------>[b,16800,2]
        # [batch_size,80*80*2+40*40*2+20*20*2,10]------>[b,16800,10]

        if self.mode == 'train':
            output = (bbox_regressions, classifications, ldm_regressions)
            #  训练时没有使用soft-max激活函数,损失函数中自带
            #  [[b,16800,4],[b,16800,2],[b,16800,10]]
        else:
